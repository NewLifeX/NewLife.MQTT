...
namespace NewLife.MqttServer
{
    ...
    public sealed class DefaultManagedMqttClient
    {
        ...
        private ConcurrentDictionary<int, INetSession> _clients = new ConcurrentDictionary<int, INetSession>();
        private ConcurrentDictionary<int, ConcurrentHashSet<string>> _subscriptions = new ConcurrentDictionary<int, ConcurrentHashSet<string>>();
        private ManualResetEventSlim _msgArrived = new ManualResetEventSlim(false);

        public DefaultManagedMqttClient()
        {
            XTrace.WriteLine("Createtetete!!!!!!!!!!!!!!!!!");
        }
