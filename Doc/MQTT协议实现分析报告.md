...
本报告对 NewLife.MQTT 项目进行全面深入分析，对标 **MQTT 3.1.1**（OASIS 标准 mqtt-v3.1.1-os）和 **MQTT 5.0**（OASIS 标准 mqtt-v5.0-os）官方协议规范，同时与社区主流 MQTT 客户端（MQTTnet、Eclipse Paho）和服务端产品（EMQX、Mosquitto、HiveMQ、VerneMQ）进行功能对比，全面列出已实现和未实现的功能。
...
- **全版本覆盖** — 支持 .NET Framework 4.5 / 4.6.1、.NET Standard 2.0/2.1、.NET 7/8/9
...
| WebHook | `Handlers` | `MqttWebHook` | 7种事件推送 |
...
| WebSocket | `WebSocket` | `WebSocketClientCodec` | ws/wss 编解码 |
| ProxyProtocol | `ProxyProtocol` | `ProxyCodec` / `ProxyMessage` | v1/v2 代理协议 |
...
| MQTT over QUIC | ? | — | .NET 9+ 平台，基于 System.Net.Quic，低延迟无队头阻塞 |
...
| 0x02 | MessageExpiryInterval | UInt32 | ? |
| 0x03 | ContentType | String | ? |
| 0x08 | ResponseTopic | String | ? |
| 0x09 | CorrelationData | Binary | ? |
| 0x0B | SubscriptionIdentifier | 变长整数 | ? |
| 0x11 | SessionExpiryInterval | UInt32 | ? |
| 0x12 | AssignedClientIdentifier | String | ? |
| 0x13 | ServerKeepAlive | UInt16 | ? |
| 0x15 | AuthenticationMethod | String | ? |
| 0x16 | AuthenticationData | Binary | ? |
| 0x17 | RequestProblemInformation | Byte | ? |
| 0x18 | WillDelayInterval | UInt32 | ? |
| 0x19 | RequestResponseInformation | Byte | ? |
| 0x1A | ResponseInformation | String | ? |
| 0x1C | ServerReference | String | ? |
| 0x1F | ReasonString | String | ? |
| 0x21 | ReceiveMaximum | UInt16 | ? |
| 0x22 | TopicAliasMaximum | UInt16 | ? |
| 0x23 | TopicAlias | UInt16 | ? |
| 0x24 | MaximumQoS | Byte | ? |
| 0x25 | RetainAvailable | Byte | ? |
| 0x26 | UserProperty | 字符串对 | ? |
| 0x27 | MaximumPacketSize | UInt32 | ? |
...
| 消息过期间隔 | ?? | `MessageExpiryInterval` 属性读写完整，分发时未检查过期（见局限） |
...
| 增强认证 | ? | `AuthMessage` 完整实现 + `AuthenticationMethod`/`AuthenticationData` 属性 |
...
| 功能 | 状态 | 说明 |
|------|------|------|
| 遗嘱延迟发布 | ?? | `WillDelayInterval` 属性读写完整，服务端当前立即发布遗嘱，未实现延迟定时器 |
| 消息过期清理 | ?? | `MessageExpiryInterval` 属性读写完整，分发时未检查是否过期 |
| 增强认证具体方法 | ?? | `AuthMessage` 框架完整（支持挑战/响应），缺少 SCRAM-SHA-256 等 SASL 具体实现 |
| 服务端 KeepAlive 覆盖 | ?? | `ServerKeepAlive` 属性可读写，服务端未在 CONNACK 中主动设置 |
...
| 会话超时清理 | ? | 30秒定时扫描 `RemoveNotAlive()`，清除不活跃会话 |
| 持久会话（内存级） | ? | `PersistentSession` 保存订阅关系和离线消息 |
...
| 会话过期间隔（5.0） | ? | `SessionExpiryInterval` 属性保存 |
| 磁盘级持久化 | ? | 内存级，可通过实现 `IMqttExchange` 扩展 |
...
| 遗嘱消息分发 | ? | 异常断开时通过 Exchange 发布 |
| 消息重发 (DUP=1) | ? | `InflightManager` 超时重发 |
| 共享订阅负载均衡 | ? | `$share/{group}/{topic}` 轮询分发 |
| $SYS 系统主题 | ? | 17种系统主题定时发布，支持订阅查询 |
| NoLocal（5.0） | ? | 不转发给发布者自身 |
| RetainAsPublished（5.0） | ? | 保留原始 Retain 标志转发 |
| RetainHandling（5.0） | ? | 3种模式：0=订阅时发送/1=仅新订阅/2=不发送 |
| 主题别名解析（5.0） | ? | 客户端→服务端别名映射 + 服务端→客户端别名分配 |
...
| 功能 | 状态 | 实现说明 |
|------|------|---------|
| 用户名/密码认证 | ? | `IMqttAuthenticator.Authenticate()` |
| TLS/SSL 加密 | ? | X509 证书，支持 pfx/pem |
| 发布权限 ACL | ? | `AuthorizePublish()` 主题级检查 |
| 订阅权限 ACL | ? | `AuthorizeSubscribe()` 主题级检查 |
| 增强认证框架（5.0） | ? | `AuthMessage` 挑战/响应流程 |
...
| 集群端口独立 | ? | 默认2883，与 MQTT 端口分离 |
| Join 加入集群 | ? | 启动时向 N-1 节点发送 Join |
| Ping 心跳保活 | ? | 15秒间隔 Ping 所有节点 |
| Leave 退出集群 | ? | 停止时通知所有节点 |
| 超时节点剔除 | ? | 根据最后活跃时间自动清理 |
| 跨节点订阅同步 | ? | `ClusterExchange.Subscribe()` 广播订阅关系 |
| 跨节点消息转发 | ? | `ClusterExchange.Publish()` 向匹配节点转发 |
| 自动节点发现 | ? | 当前仅支持手动配置 |
...
| 功能 | 状态 | 实现说明 |
|------|------|---------|
| **消息桥接（双向）** | ? | `MqttBridge` 支持 In/Out/Both 三种方向，主题前缀映射，QoS 上限控制 |
| **规则引擎（5种动作）** | ? | Republish 转发 / WebHook 触发 / Bridge 桥接 / Drop 丢弃 / Custom 自定义委托 |
| **WebHook 事件推送** | ? | 7种事件：连接/断开/发布/投递/订阅/退订，异步非阻塞，支持重试 |
| **运行统计** | ? | `MqttStats` 连接数/消息数/字节数/订阅数/速率，1秒滑动窗口 |
| **$SYS 系统主题** | ? | 17种标准主题，可配发布间隔（默认60秒） |
...
| TLS/SSL 连接 | ? | 设置 `SslProtocol` + `Certificate` |
| WebSocket 连接 | ? | `ws://host:port/path` |
| WSS 连接 | ? | `wss://host:port/path`，自动启用 TLS |
| QUIC 连接 | ? | `MqttQuicClient`，.NET 9+ 平台 |
| 可靠 UDP 连接 | ? | `MqttUdpClient`，全平台 |
| 连接字符串初始化 | ? | `Init("Server=;UserName=;Password=")` |
...
| 断线重连（指数退避） | ? | `2^n` 退避策略，可配初始/最大延迟/最大次数 |
| 重连后自动重订阅 | ? | 维护 `_subs` 字典，重连后自动重发 |
| 心跳保活 | ? | KeepAlive 半周期定时 Ping |
| 消息重发（Inflight） | ? | `InflightManager` DUP=1 超时重发 |
| 编码器可替换 | ? | `IPacketEncoder Encoder`，默认 JSON |
| 性能跟踪（ITracer） | ? | 每个发送/接收操作均有 Span 埋点 |
| NoDelay 低延迟 | ? | TCP 连接默认关闭 Nagle 算法 |
| 重复订阅防护 | ? | `_subs` 字典去重 |
| 连接/断开事件 | ? | `Connected`/`Disconnected` 事件 |
| 收到消息事件 | ? | `Received` 事件（全局） + 订阅回调（按主题） |
| 超时销毁重建连接 | ? | 连续3次超时自动销毁，下次请求重建 |
| 阿里云 IoT 适配 | ? | `AliyunMqttClient` 内置 HMAC 签名 + 属性上报 + 时钟同步 |
...
| **MIT 开源协议** | 最宽松的开源协议，可自由修改商用，无 copyleft 限制，无专利条款 |
| **客户端+服务端一体** | 单一 `NewLife.MQTT` NuGet 包同时提供客户端和服务端，极简集成 |
| **全版本 .NET 覆盖** | .NET Framework 4.5 ~ .NET 9，覆盖所有主流 .NET 运行时 |
| **六种传输协议** | TCP/TLS/WS/WSS/QUIC/可靠UDP，业界覆盖面最广 |
| **MQTT over QUIC** | 业界罕见，仅 EMQX 和 NewLife.MQTT 支持，低延迟无队头阻塞 |
| **MQTT over 可靠UDP** | 独创自研，无需操作系统 QUIC 支持，全平台可用 |
| **企业功能免费内置** | 桥接/规则引擎/WebHook/统计/$SYS，竞品需企业版或付费插件 |
| **纯国产自主研发** | 完全自主知识产权，无外部依赖法律风险，信创合规 |
| **ProxyProtocol 支持** | 支持 nginx/HAProxy 代理协议，透传真实客户端 IP |
| **阿里云 IoT 内置适配** | `AliyunMqttClient` 开箱即用，无需额外适配 |
| **DI 扩展架构** | 基于依赖注入的可扩展架构，可替换认证器、交换机、处理器 |
